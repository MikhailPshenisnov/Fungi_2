...
## Фикс бэка для мобилки

- [x] Зафиксировать единый auth-контракт для web + mobile: API принимает JWT только через `Authorization: Bearer <token>`.
- [x] Переписать `POST /Authorization/ValidateToken` под bearer-схему: убрать зависимость от `Request.Cookies`, получать токен из `Authorization` header, вернуть `tokenData` по текущему пользователю.
- [x] Убрать legacy-ветку в `ValidateToken` с токеном из body (`ValidateTokenRequest.Token`) или вынести в отдельный защищенный admin-only endpoint, если реально нужен сценарий валидации чужого токена.
- [x] Привести `AuthorizationController` к одной модели авторизации: перестать использовать cookie как источник истины для auth-потока.
- [x] Обновить CORS-конфигурацию для dev-сценария мобилки в браузере: обеспечить прохождение запросов с `http://localhost:8081` без ручного добавления порта в репозиторий.
- [x] Убрать необходимость пушить порты в репозиторий: в `Development` разрешать только `localhost/127.0.0.1` (любой порт) через CORS-предикат, а в non-dev оставлять строгий allowlist доменов окружения.
- [x] Синхронизировать OpenAPI/Swagger-описание `ValidateToken`: указать bearer в header как обязательный способ передачи токена.
- [x] Добавить проверки после правок:
  - без `Authorization` -> `401 Unauthorized`,
  - с валидным bearer -> `200 OK` и корректный `tokenData`,
  - preflight с `Origin: http://localhost:8081` проходит без CORS-ошибок.
- [x] Подготовить короткий changelog для команды мобилки: что изменилось в контракте, какой формат запроса обязателен, какие старые сценарии удалены/deprecated.

## Внедрение MkDocs (единая документация)

- [x] Создать базовую структуру документации в репозитории: `docs/` + `mkdocs.yml`.
- [x] Подключить тему `mkdocs-material` и минимальные плагины (`search`, `include` при необходимости).
- [x] Определить структуру разделов: `Getting Started`, `Architecture`, `Backend API`, `Frontend`, `Mobile`, `Workflow`, `Changelog`.
- [x] Перенести и нормализовать текущую документацию из `README.md` и `QuickStart/QuickStart.md` (убрать дубли, оставить ссылки на единый источник).
- [x] Оставить Swagger/OpenAPI источником API-контракта и добавить в docs ссылку/встраивание актуального `swagger.json`.
- [x] Добавить правила сопровождения документации: где хранится truth, как обновлять docs при изменении API/архитектуры.
- [x] Добавить локальный запуск docs (`make/docs` или npm/pip script) и короткую инструкцию для команды.
- [x] Настроить CI-проверку сборки документации (`mkdocs build`) на Pull Request.
- [ ] Настроить публикацию документации (GitHub Pages/Netlify/Vercel) и добавить ссылку в `README.md`.
- [x] Провести первичную ревизию контента после переноса: актуальность auth/cors контрактов, onboarding, workflow, known issues.
